...
{
    public class IntegrationEventLogService<TContext> : IDisposable, IIntegrationEventLogService 
        where TContext : DbContext
    {
        ...
        {
            var result = _context.Set<IntegrationEventLogEntry>()
        ...
        {
            var eventLogEntry = _context.Set<IntegrationEventLogEntry>().Single(e => e.EventId == eventId);
            eventLogEntry.State = status;

            if (eventLogEntry.State == EventStateEnum.InProgress)
            {
